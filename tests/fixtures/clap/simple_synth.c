/*
 * Simple CLAP Synth - Actually makes sound!
 * A basic monophonic sine wave synth.
 */
#include <string.h>
#include <stdlib.h>
#include <stdio.h>
...
/* Audio ports extension - output only */
...
static clap_process_status plugin_process(const clap_plugin_t *plugin, const clap_process_t *process) {
    ...
    for (uint32_t i = 0; i < event_count; i++) {
        ...
        if (hdr->type == CLAP_EVENT_MIDI) {
            ...
            if (status == 0x90 && vel > 0) {
                ...
                data->freq = note_to_freq(note);
                data->volume = vel / 127.0 * 0.3;
            } else if (status == 0x80 || (status == 0x90 && vel == 0)) {
                /* Note off */
                if (note == data->current_note) {
                    data->note_on = 0;
                }
            }
        }
    ...
    double phase_inc = 2.0 * M_PI * data->freq / data->sample_rate;

    for (uint32_t i = 0; i < frames; i++) {
        float sample = 0.0f;

        if (data->note_on) {
            sample = (float)(sin(data->phase) * data->volume);
            data->phase += phase_inc;
